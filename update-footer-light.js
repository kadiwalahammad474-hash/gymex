...
const dirs = [
  'index', 'about', 'business', 'contact', 'features', 'pricing', 'career'
];
...
<div style="background:#fff; padding:50px 0 30px;">
  <div style="max-width:1200px; margin:0 auto; padding:0 24px; display:flex; justify-content:space-between; gap:40px;">
    ...
    <div style="flex:0.5;"><h3 style="color:#222; font-size:18px; font-weight:600; margin-bottom:14px;">Follow Us On</h3><div style="display:flex; gap:14px; margin-top:4px;"><a href="https://www.facebook.com/gymexsoftware" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="#3b5998"><circle cx="12" cy="12" r="12" fill="#3b5998"/><path d="M16.67 15.04l.52-3.39h-3.25v-2.2c0-.93.46-1.84 1.92-1.84h1.49V4.12s-1.35-.23-2.64-.23c-2.7 0-4.46 1.63-4.46 4.58v2.62H7.6v3.39h2.65V22h3.35v-6.96h2.36z" fill="white"/></svg></a><a href="https://www.instagram.com/gymexclubmanagement" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24"><defs><radialGradient id="igGradFoot" cx="30%" cy="107%"><stop offset="0%" stop-color="#fdf497"/><stop offset="5%" stop-color="#fdf497"/><stop offset="45%" stop-color="#fd5949"/><stop offset="60%" stop-color="#d6249f"/><stop offset="90%" stop-color="#285AEB"/></radialGradient></defs><circle cx="12" cy="12" r="12" fill="url(#igGradFoot)"/><rect x="5" y="5" width="14" height="14" rx="4" fill="none" stroke="white" stroke-width="1.5"/><circle cx="12" cy="12" r="3.5" fill="none" stroke="white" stroke-width="1.5"/><circle cx="17" cy="7" r="1" fill="white"/></svg></a><a href="https://www.linkedin.com/company/gymex-club-management-software/" target="_blank"><svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24"><circle cx="12" cy="12" r="12" fill="#0077b5"/><path d="M8 10.5v5.5M8 8v.01M11 16v-3.5c0-1.1.9-2 2-2s2 .9 2 2V16" stroke="white" stroke-width="1.8" fill="none" stroke-linecap="round"/><rect x="5.5" y="10" width="5" height="5.5" rx="0.5" fill="none" stroke="white" stroke-width="1.3"/></svg></a></div></div>
  </div>
</div>
...
let updated = 0;
dirs.forEach(dir => {
  const file = path.join(dir, dir + '.html');
  try {
    ...
    if (footerStart === -1) { console.log(file + ': no footer tag'); return; }
    
    const footerEnd = html.indexOf('</footer>', footerStart);
    if (footerEnd === -1) { console.log(file + ': no closing footer'); return; }
    
    // Skip any extra divs after old footer
    let afterFooter = footerEnd + 9;
    const divPattern = /<\/div>\s*/g;
    // Don't be too aggressive, just replace footer to </footer>
...
// Also do contact page which already has the grey bar above
try {
  let contactHtml = fs.readFileSync('contact/contact.html', 'utf8');
  const footerStart = contactHtml.indexOf('<footer class="footer"');
  if (footerStart !== -1) {
    const footerEnd = contactHtml.indexOf('</footer>', footerStart);
    if (footerEnd !== -1) {
      let afterFooter = footerEnd + 9;
      // Remove duplicate grey bar from contact - it already has one above the contact form
      contactHtml = contactHtml.substring(0, footerStart) + newFooter + contactHtml.substring(afterFooter);
      fs.writeFileSync('contact/contact.html', contactHtml, 'utf8');
      console.log('contact/contact.html: UPDATED');
      updated++;
    }
  }
} catch(e) {
